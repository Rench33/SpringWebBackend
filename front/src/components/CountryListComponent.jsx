...
const CountryListComponent = props => {
    ...
    const setChecked = v => {
        const length = countries?.length || 0; // Используем опциональную цепочку
        setCheckedItems(Array(length).fill(v));
    }
    ...
    const refreshCountries = (cp = 0) => {
        BackendService.retrieveAllCountries(cp, limit)
            .then(resp => {
                ...
                // Если данные в resp.data, а не в resp.data.content:
                setCountries(resp.data || []); // Исправлено здесь
                setTotalCount(resp.data?.length || 0); // Или другой ключ, если есть пагинация
                setPage(cp);
            })
            .catch(error => {
            ...
            <PaginationComponent
                ...
                <table className="table table-sm">
                    ...
                    <tbody>
                        {countries.length === 0 ? (
                            <tr>
                                <td colSpan="2">Список стран пуст</td>
                            </tr>
                        ) : (
                            countries.map((country, index) => (
                                <tr key={country.id}>
                                    <td>{country?.name || "Нет названия"}</td>
                                    <td>
                                        <div className="btn-toolbar">
                                            <div className="btn-group ms-auto">
                                                <button
